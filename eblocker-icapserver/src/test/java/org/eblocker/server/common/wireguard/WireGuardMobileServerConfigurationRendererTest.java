...
public class WireGuardMobileServerConfigurationRendererTest {
    ...
    @Test
    public void renderSanitizesDeviceIdComment() {
        WireGuardMobileServer server = new WireGuardMobileServer();
        server.setPrivateKey("server-private");
        server.setAddress("10.9.0.1/24");
        WireGuardMobilePeer peer = new WireGuardMobilePeer(1, "device\nPostUp = bad#x");
        peer.setPublicKey("peer-public");
        peer.setAddress("10.9.0.2/32");

        String config = new WireGuardMobileServerConfigurationRenderer().render(server, java.util.List.of(peer), 51820);

        Assert.assertTrue(config.contains("# device_PostUp = bad_x"));
        Assert.assertFalse(config.contains("\nPostUp = bad"));
    }
}
