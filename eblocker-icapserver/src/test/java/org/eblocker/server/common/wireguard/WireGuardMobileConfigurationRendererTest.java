...
public class WireGuardMobileConfigurationRendererTest {
    ...
    @Test
    public void renderBracketsIpv6EndpointAndStripsLineBreaks() {
        WireGuardMobileServer server = new WireGuardMobileServer();
        server.setPublicKey("server-public");
        WireGuardMobilePeer peer = new WireGuardMobilePeer(1, "device");
        peer.setPrivateKey("peer-private");
        peer.setAddress("10.9.0.2/32");

        String config = new WireGuardMobileConfigurationRenderer().render(
                server, peer, "2001:db8::1\nInjected = bad", 51820, "10.9.0.1", "0.0.0.0/0", 25);

        Assert.assertTrue(config.contains("Endpoint = [2001:db8::1Injected = bad]:51820"));
