 ...
 *
 * Provider profiles are stored and executed as WireGuard configurations.
 */
public class WireGuardControllerImpl implements WireGuardController {
    ...
    @Inject
    public WireGuardControllerImpl(AnonymousService anonymousService, WireGuardService wireGuardService,
    ...
    public VpnProfile createProfile(Request request, Response response) {
        ...
        try {
            ...
            response.addHeader("Location", "/anonymous/wireguard/profile/" + profile.getId());
            return profile;
        } catch (IOException e) {
    ...
    public OpenVpnConfigurationViewModel uploadProfileConfigOption(Request request, Response response) {
        throw new BadRequestException("WireGuard provider configurations do not use external option files");
    }
