...
public class WireGuardMobileConfigurationRenderer {
    public String render(WireGuardMobileServer server, WireGuardMobilePeer peer, String endpointHost, int endpointPort,
        ...
        builder.append("Endpoint = ").append(formatEndpoint(endpointHost, endpointPort)).append('\n');
    ...
    private String formatEndpoint(String endpointHost, int endpointPort) {
        String sanitizedHost = stripLineBreaks(endpointHost);
        if (sanitizedHost.indexOf(':') >= 0 && !sanitizedHost.startsWith("[") && !sanitizedHost.endsWith("]")) {
            sanitizedHost = "[" + sanitizedHost + "]";
        }
        return sanitizedHost + ":" + endpointPort;
    }
