...
public class WireGuardService {
    ...
    private void reconfigureAclsAndRouting(int profileId, ClientState state) {
        persistWireGuardClientState(profileId, state);
    ...
    private void persistWireGuardClientState(int profileId, ClientState state) {
        if (!state.active || state.devicesById.isEmpty()) {
            dataSource.delete(OpenVpnClientState.class, profileId);
            return;
        }

        OpenVpnClientState vpnClientState = new OpenVpnClientState();
        vpnClientState.setId(profileId);
        vpnClientState.setState(OpenVpnClientState.State.ACTIVE);
    ...
    private int nextSharedProfileId() {
        int nextWireGuardId = dataSource.nextId(WireGuardProfile.class);
        ...
        int id = Math.max(nextWireGuardId, Math.max(maxOpenVpnId, maxWireGuardId) + 1);
        dataSource.setIdSequence(WireGuardProfile.class, id);
        return id;
    }
